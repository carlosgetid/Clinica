...
public class GUIConsultaMOD extends JDialog implements ItemListener,ActionListener{
	...
	public static void main(String[] args) {
		try {
			GUIConsultaMOD dialog = new GUIConsultaMOD(null,null);
	 ...
	 * Create the dialog.
	 */
	public GUIConsultaMOD(JTable TbCon,Consulta cta){
		Tbconsul = TbCon;
		con=cta;
		...
		btnModificarConsulta.addActionListener(this);
		btnModificarConsulta.setFont(new Font("Tahoma", Font.PLAIN, 12));
		btnModificarConsulta.setBounds(270, 243, 142, 31);
		...
		for (int i = 0; i < GuiPrincipal.ar.tamaño(); i++) {
			...
			if(r.getcodigoConsulta()==con.getCodigoConsulta()){
				...
				Object row[]={r.getCodigoMedicina(),
						...
						r.getPrecioUnitario(),
						r.getCantidad(),
						(r.getPrecioUnitario()*r.getCantidad())};
				Tmodel.addRow(row);
			}
		}
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==btnModificarConsulta){
			APbtnModificarConsulta(e);
		}
	}
	protected void APbtnCreceta(ActionEvent g0){
		if(LeerTotalPagar()<100){
			Libreria.Mensaje(null,this,"El monto minimo debe ser de 100.00", "Error", -1, "images/no ts.png");
			}
		else{
			Listar();
	...
	protected void APbtnModificarConsulta(ActionEvent g3){
		int rc= tbMedicinas.getRowCount();
		if(rc>0){
			con.setFechaAtencion(LeerFechaAtencion());
			con.setHoraAtencion(LeerHoraAtencion());
			con.setTotalPagar(LeerTotalPagar());
			con.setEstado(cboEstado.getSelectedIndex());
			for (int i = 0; i < GuiPrincipal.ar.tamaño(); i++) {
				Receta re= GuiPrincipal.ar.obtener(i);
				int cr= re.getcodigoConsulta();
				if(con.getCodigoConsulta()==cr){
			...
			Libreria.Mensaje(null,this, "Datos Modificados Correctamente", "Adicionar", -1, "images/ok t.png");
			grabarConsultatxt();
			CerrarCR();
		}
		else{
	...
	private void ConfirmarCR(){
		int ok= Libreria.confirmacion(null, this , "¿Realmente desea cancelar la modificación?", "Salir", "images/shutdown t.png");
		if(ok==0){
			CerrarCR();
		}
	}
